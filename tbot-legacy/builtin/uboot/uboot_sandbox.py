...
def uboot_sandbox(
    ...
    *,
    ...
    :param str repo: URI of the U-Boot repository, defaults to
                 ``tb.config["uboot.repository"]``
    :param pathlib.PurePosixPath builddir: Where to build U-Boot, defaults to
                     ``tb.config["tbot.workdir"] / "uboot-sandbox"``
    :param pathlib.PurePosixPath patchdir: Optional directory of patches to be applied. If this
...
"""
    ...
    if tb.config["uboot.test.use_venv", True]:
        tbot.log.doc(
            """Create a virtualenv and install pytest inside it:
"""
        )
        ...
        with tb.machine(tbot.machine.MachineLabEnv()) as tb:
            ...
            tbot.log.doc(
                f"""Now clean the U-Boot repository and start the sandbox testsuite.
"""
            )
